...
const API_URL = `${import.meta.env.VITE_API_URL}/api/tutor/disputes`;
const BOOKINGS_URL = `${import.meta.env.VITE_API_URL}/api/tutor/bookings-for-dispute`;
const CREATE_URL = `${import.meta.env.VITE_API_URL}/api/tutor/disputes`;
...
const DisputeMessageForm = ({ disputeId, onMessageSent }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/tutor/disputes/${disputeId}/message`, {
        method: 'POST',
        headers: {
                        ...
                        <select className="form-select" required value={form.bookingId} onChange={e => setForm(f => ({ ...f, bookingId: e.target.value }))} style={{ borderColor: '#e2e8f0', borderRadius: '0.5rem' }}>
                          ...
                          {bookings.map(b => (
                            <option key={b._id} value={b._id}>
                              {b.subject?.name} | {new Date(b.sessionTime).toLocaleString()} | Parent: {b.parentId?.name}
                            </option>
                          ))}
                        </select>
                      </div>
